/**
 * Copyright 2009 Wind River Systems Inc. All rights reserved.
 */
...
// RNR : delete this            dialog.setPreferenceStore(createPreferenceStore(bpContext));
    ...
    private CBreakpointContext getCBreakpointContext() {
        ...
        if (ss.size() == 1 && ss.getFirstElement() instanceof ICBreakpoint) {
            ...
            List<ICBreakpoint> selectionList = new ArrayList<ICBreakpoint>(ss.size());
            for (Iterator<?> itr = ss.iterator(); itr.hasNext();) {
                Object next = itr.next(); 
                if (next instanceof ICBreakpoint) {
                    selectionList.add((ICBreakpoint)next);
                }
            }
            return new CBreakpointContext(
                selectionList.toArray(new ICBreakpoint[selectionList.size()]), 
                fDebugContextProvider.getActiveContext());
        }
        
    }
    
    private CBreakpointPreferenceStore createPreferenceStore(CBreakpointContext bpContext) {
        CBreakpointPreferenceStore preferenceStore = new CBreakpointPreferenceStore();
        preferenceStore.setContext(bpContext);
